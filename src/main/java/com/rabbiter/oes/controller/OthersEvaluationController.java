...
//    }
     ...
     */
    @PutMapping("/option")
    public ApiResult optionManage(@RequestBody OtherScore otherScore) {
        ...
        if (oc == null) {
            for (i = 0; i <= otherScore.getOptionList().size(); i++) {
                ...
                //更新score_manage表中的score总分以及五类分项得分
            ...
            for (i = 0; i <= otherScore.getOptionList().size(); i++) {
                ...
                othersEvaluationService.updateOptionScore(otherScore.getPjid(), otherScore.getBpjid(), otherScore.getScore());
    ...
    public int updateScoreManage(OtherScore otherScore) {
        return othersEvaluationService.updateScoreManage(otherScore);
    }
