...
//            "from score_manage where pj_id = #{userId} and NOT (ISNULL(score) or score = '') order by bpj_uass")
    @Select("select bpj_name bpjName,bpj_id bpjId,score,bpj_instname instname,level,submit,A AS scoreA,B AS scoreB,C AS scoreC,D AS scoreD,E AS scoreE " +
            "from score_manage where pj_id = #{userId} and NOT (ISNULL(score) or score = '') order by score desc")
...
//    int updataOthersScore(OtherScore otherScore);
    ...
    @Update("update leaderinfo t set scoreA = (select avg(A) from score_manage where bpj_id = t.id and not(ISNULL(A) or A = ''))," +
            "scoreB = (select avg(B) from score_manage where bpj_id = t.id and not(ISNULL(B) or B = ''))," +
            "scoreC = (select avg(C) from score_manage where bpj_id = t.id and not(ISNULL(C) or C = ''))," +
            "scoreD = (select avg(D) from score_manage where bpj_id = t.id and not(ISNULL(D) or D = ''))," +
            "scoreE = (select avg(E) from score_manage where bpj_id = t.id and not(ISNULL(E) or E = ''))" +
            "where exists (select 1 from score_manage where bpj_id = t.id)")
    ...
    @Insert("insert into optioninfo (pjId,bpjId,quId,optioninfo.option,score) values (#{pjId},#{bpjId},#{quId},#{option},#{score})")
    int insertOption(String pjId,String bpjId,int quId,int option,String score);

    //更新optioninfo表中的题目的选项
    @Update("update optioninfo set optioninfo.option = #{option} where pjId = #{pjId} and bpjId = #{bpjId} and quId = #{quId}")
